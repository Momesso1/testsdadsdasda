...
**Desafio:** O `MoveGroupInterface` precisa processar callbacks (como o estado das juntas) enquanto o construtor ainda está rodando. Caso seja utilizado o mesmo nó/executor, ocorre um *Deadlock*.

**Solução:**
1.  Cria-se um nó separado (`moveit_node_`) apenas para o MoveIt.
2.  Cria-se um `executor_` separado.
3.  Lança-se uma `std::thread` dedicada para rodar esse executor.
...
Ao desligar o nó, é preciso parar o executor e juntar a thread para evitar erros de memória (*Segmentation Fault*).
...
O MoveIt pode demorar para subir. Utiliza-se um `Timer` no construtor para chamar esta função 1 segundo após o boot.
Aqui são instanciadas as interfaces de controle do braço (`panda_arm`) e da garra (`hand`).
...
Quando o movimento não precisa ser reto (ex: ir de uma caixa para outra), utiliza-se o planejador probabilístico `RRTConnect`. Ele desvia de obstáculos automaticamente.
...
Aqui gerencia-se o ciclo de vida da Action ROS 2 (`PickObject`).
